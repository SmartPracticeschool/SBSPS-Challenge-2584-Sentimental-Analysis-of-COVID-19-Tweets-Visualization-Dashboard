...
sidebar = html.Div(
    [
        html.H4("Dashboard", className="display-6"),
        html.Hr(),
        html.P(
        ...
        dbc.Nav(
            [
				...
				dbc.NavLink("User Mentions", href="/page-6", id="page-6-link")
			],
            vertical=True,
            pills=True,
        ),
...
html.H4("Sentiment Analysis of COVID-19 Tweets"),
	...
	html.Div(id='us-output-container')
]) 
...
def update_output(value):
    ...
    hst = html.Div([
		#html.H6('Age', style={'textAlign': 'center'}),
            ...
            figure={
                'data': [
				{'x': hs_cnt['hashtag'],'y':hs_cnt['hst_count'], 'type': 'bar', 'name': 'Sentiment'}
                ],
            'layout': {
				'plot_bgcolor': colors['background'],
				'paper_bgcolor': colors['background'],
...
@app.callback(
    dash.dependencies.Output('us-output-container', 'children'),
    [dash.dependencies.Input('us-dropdown', 'value')])
def update_output(value):
    df['Tweets'] = df['Tweets'].str.replace('\d+','')
    UM = df[df['Sentiment'] == value]
    idx = UM.index
    new_ment = []
    for i in idx:
    ...
    usm = html.Div([
		#html.H6('Age', style={'textAlign': 'center'}),
            ...
            figure={
                'data': [
				{'x': top_ments['ments'],'y':top_ments['ments_count'], 'type': 'bar', 'name': 'Sentiment'}
                ],
            'layout': {
				'plot_bgcolor': colors['background'],
				'paper_bgcolor': colors['background'],
				...
				'font': {
				'color': colors['text']},
				'title': 'User Mentions vs Count',
				'xaxis':{
                       'title':'User Mentions'
                        },
                'yaxis':{
                       'title':'User Mentions Count'
                       }
				}
                }
                )
