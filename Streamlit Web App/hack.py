...
word_sentiment = st.sidebar.radio('Display hashtags for which sentiment?', ('Positive', 'Neutral', 'Negative'))
if not st.sidebar.checkbox("Lock up", False):
    ...
    HT = data[data['Sentiment'] == word_sentiment]
    ind = HT.index
    new_lst = []
    for i in ind:
        hst = [i  for i in HT['Tweets'][i].split() if i.startswith("#")]
        new_lst.append(hst)
    hashtags = pd.DataFrame({"hash":new_lst})
    s = hashtags.apply(lambda x: pd.Series(x['hash']),axis=1).stack().reset_index(level=1, drop=True)
    s.name = 'hash'
    hs = hashtags.drop('hash', axis=1).join(s)
    hs = hs.dropna()
    hs['hash'] = hs['hash'].str.replace(':', '')
    hs['hash'] = hs['hash'].str.replace('Â', '')
    hs_count = hs['hash'].value_counts()
    hs_count = pd.DataFrame({'hashtag':hs_count.index, 'hst_count':hs_count.values})
    hs_cnt = hs_count[:15]
    fig_hs = px.bar(hs_cnt, x='hashtag', y='hst_count',color='hst_count',text='hst_count', width = 800, height=500)
    st.plotly_chart(fig_hs)
    
    
st.sidebar.header("User Mentions")
word_sentiment = st.sidebar.radio('Display user mentions for which sentiment?', ('Positive', 'Neutral', 'Negative'))
if not st.sidebar.checkbox("Shut", False):
    st.subheader('User Mentions in %s Sentiment Tweets' % (word_sentiment))
    #st.success("Hashtags can help grow our digital social circles. The best hashtags are the ones most relevant to the post, and the ones that will reach the largest number of viewers.")
    UM = data[data['Sentiment'] == word_sentiment]
    idx = UM.index
    new_ment = []
    for i in idx:
        ment = [i  for i in UM['Tweets'][i].split() if i.startswith("@")]
        new_ment.append(ment)
    ment = pd.DataFrame({"mnt":new_ment})
    m = ment.apply(lambda y: pd.Series(y['mnt']),axis=1).stack().reset_index(level=1, drop=True)
    m.name = 'ment'
    ments = ment.drop('mnt', axis=1).join(m)
    ments = ments.dropna()
    ments['ment'] = ments['ment'].str.replace(':', '')
    ments_count = ments['ment'].value_counts()
    ments_count = pd.DataFrame({'ments':ments_count.index, 'ments_count':ments_count.values})
    top_ments = ments_count[:15]
    fig_mnt = px.bar(top_ments, x='ments', y='ments_count',color='ments_count',text='ments_count', height=500)	
    st.plotly_chart(fig_mnt)
